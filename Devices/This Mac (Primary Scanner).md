---
type: device
status: online
mode: active
latency_ms: 0.3
packet_loss_pct: 0.0
tags: [homelab, homeassistant]
generated: 2026-06-09 23:10:49
---

# This Mac (Primary Scanner)

> Daily driver · mf-mac-905

| Field | Value |
|-------|-------|
| **Reachability** | **online** |
| **Mode** | **active** |
| **Ping avg** | 0.3 ms |
| **Packet loss** | 0.0% |
| **Address** | 10.0.0.227 |
| **Tailscale** | 100.88.199.80 |
| **HA state** | — |

## Hardware
MF-MAC-905 · marshalls

## Services
- Obsidian
- Cursor
- Tailscale

## Connections
| Link | Medium | Direction | Traffic | Path | Notes |
|------|--------|-----------|---------|------|-------|
| ↔ [[Devices/Router Gateway|Router Gateway]] | Wi-Fi | bidirectional | routing | up | Daily driver Mac |
| → [[Devices/ZBook Server|ZBook Server]] | Wi-Fi | bidirectional | management | up | SSH · SMB · HA UI |
| → [[Devices/Home Assistant|Home Assistant]] | HTTP / Web UI | bidirectional | management | up | Dashboard · Obsidian refresh |
| → [[Devices/ZBook Server|ZBook Server]] | Tailscale overlay | bidirectional | routing | up | Remote access overlay |
